...
function drawEhcbNameplateBg(ctx,x,y,w,h,c,sc,thumb,variant='pro'){
  ...
  // Rechte Logo-Zone als kontrollierter roter Keil statt unruhiger Fläche.
  const rx=x+w*.78;
  ...
  logoShade.addColorStop(0,'rgba(0,0,0,.28)');
  logoShade.addColorStop(.62,'rgba(0,0,0,.12)');
  logoShade.addColorStop(1,'rgba(0,0,0,0)');
  ctx.fillStyle=logoShade;
  ctx.fillRect(x+w*.7,y,w*.3,h);
  ...
  ctx.lineWidth=Z(1);
  for(let i=0;i<18;i++){
    const px=x+w*(.78+((i*37)%20)/100);
    const py=y+h*((i*19)%100)/100;
    ctx.beginPath();
    ctx.moveTo(px,py);
    ctx.lineTo(px+Z(22+(i%5)*9),py-Z(4+(i%4)*3));
    ctx.stroke();
  }
  ctx.restore();

  // Dünne Kanten oben/unten für saubere Druckwirkung.
  ctx.fillStyle='rgba(255,255,255,.08)';
  ctx.fillRect(x,y,Z(0)+w,Z(1.3));
  ctx.fillStyle='rgba(0,0,0,.45)';
  ctx.fillRect(x,y+h-Z(3),w,Z(3));
  ctx.strokeStyle='rgba(212,185,106,.18)';
  ctx.lineWidth=Z(1);
  ctx.beginPath();
  ctx.moveTo(x,y+Z(5));
  ctx.lineTo(x+w,y+Z(5));
  ctx.moveTo(x,y+h-Z(6));
  ctx.lineTo(x+w,y+h-Z(6));
  ctx.stroke();
}
...
function sheetLayout(sheetKey){
  ...
  function calc(sw,sh){
    function fitCount(page,plate){
      const withMargin=Math.floor((page-2*m+g)/(plate+g));
      if(withMargin>0)return withMargin;
      // 289 mm passt auf A4 quer nur mit ca. 4 mm Seitenrand.
      return plate<=page?1:0;
    }
    const cols=Math.max(0,fitCount(sw,pw));
    const rows=Math.max(0,fitCount(sh,ph));
    const gridW=cols*pw+Math.max(0,cols-1)*g;
    const gridH=rows*ph+Math.max(0,rows-1)*g;
    return{
      cols,rows,perPage:cols*rows,sw,sh,gridW,gridH,
      originX:(sw-gridW)/2,
      originY:(sh-gridH)/2,
    };
  }
...
function drawCutMarks(doc,x,y,w,h,pageW=null,pageH=null){
  if(S.pdfCutMarks===false)return;
  const L=CUT_MARK_LEN_MM,o=CUT_MARK_OFFSET_MM;
  const mx=x+w/2,my=y+h/2;
  const canLeft=pageW==null||x-o-L>=0;
  const canRight=pageW==null||x+w+o+L<=pageW;
  const canTop=pageH==null||y-o-L>=0;
